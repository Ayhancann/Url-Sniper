...
clear()
print01(Fore.RED + "Ayhancann tarafından yapılmıştır!" + Fore.RESET + "\n")
time.sleep(1)
print01(Fore.CYAN + "Destek sunucusu: https://discord.gg/wkw6PFtM" + Fore.RESET + "\n\n")
time.sleep(5)
...
def snipe(value):
    ...
    try:
        ...
        if json_response.get("code") == 50001:
            print(f"{Fore.MAGENTA}[{get_time()}] {Fore.RESET} || {Fore.RED}[TOKEN YETKİSİ YOK]")
            return
        ...
        if json_response.get("code") == value[0]:
            print("[{}] Snipe atıldı || [{}]".format(get_time(), value[0]))
            ...
            if "retry_after" in json_response:
                ...
                ctypes.windll.kernel32.SetConsoleTitleW(f"Vanity Spammer  | ayhancan || Tokens: [{num_symbols}] || Servers: [{num_servers}] || Attempts: [{reqs}] || Speed: {speed:.2f} requests/sec || Time Left: {get_cooldown(time.time() + json_response['retry_after'])}")
...
print(f"{Fore.MAGENTA}[{get_time()}] {Fore.RESET}|| {Fore.GREEN}Snipelamaya başla {Fore.RESET}|| {Fore.RED}Hız: [{interval}ms]\n")
time.sleep(2)
